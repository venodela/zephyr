					...
					uint16_t *data)
{
	...
	if (ret) return ret;
	ret = mdio_read(config->mdio_dev, config->addr, MMD_CTRL_DATA, data);
	return ret;
}
					 ...
					 uint16_t data)
{
        ...
        ret = mdio_write(config->mdio_dev, config->addr, MMD_CTRL_ADDR,
			 DP83867_REGCR_FUNC_DATA_NO_POSTINC | DP83867_CTRL);
        if (ret) return ret;
        ret = mdio_write(config->mdio_dev, config->addr, MMD_CTRL_DATA, data);
        return ret;
}
...
static int phy_ti_dp83867_init(const struct device *dev)
{
	...
	uint8_t fifo_depth = (phycr_val & PHY_TI_DP83867_FIFO_DEPTH_MASK) >> PHY_TI_DP83867_FIFO_DEPTH_SHIFT;
