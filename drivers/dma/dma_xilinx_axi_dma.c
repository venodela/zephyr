...
static int dma_xilinx_axi_dma_start(const struct device *dev, uint32_t channel)
{
	...
	dma_xilinx_axi_dma_write_reg(channel_data, XILINX_AXI_DMA_REG_TAILDESC,
				     (uint32_t)(((uintptr_t)current_descriptor) & 0xffffffffU));
...
#ifdef CONFIG_DMA_64BIT
	current_descriptor->buffer_address = (uint32_t)buffer_addr & 0xffffffffU;
					...
					struct dma_config *dma_cfg)
{
	...
	for (int i = 0; i < data->channels[channel].num_descriptors; i++) {
		...
		low_bytes = (uint32_t)(((uint64_t)nextdesc) & 0xffffffffU);
		data->channels[channel].descriptors[i].nxtdesc = low_bytes;

#ifdef CONFIG_DMA_64BIT
		high_bytes = (uint32_t)(((uint64_t)nextdesc >> 32) & 0xffffffffU);
	...
	dma_xilinx_axi_dma_write_reg(
		...
		(uint32_t)(((uintptr_t)&data->channels[channel].descriptors[0]) & 0xffffffffU));
	dma_xilinx_axi_dma_write_reg(
